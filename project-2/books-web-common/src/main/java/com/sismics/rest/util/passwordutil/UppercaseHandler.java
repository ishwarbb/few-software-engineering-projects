...
public class UppercaseHandler implements PasswordHandler {
    ...
    public void handleRequest(String password, String name) throws JSONException {
        if (!password.matches(".*[A-Z]+.*")) {
            throw new ClientException("ValidationError", MessageFormat.format("{0} must contain at least one uppercase letter", name));
        }
        if (nextHandler != null) {
